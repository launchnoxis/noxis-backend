...
const localLaunchSchema = Joi.object({
...
router.get('/test-pumpportal', async (req, res) => {
  try {
    ...
    res.json({ status: response.status, ok: response.ok, bodyLength: text.length, bodyPreview: text.slice(0, 200) });
  } catch (err) {
...
router.get('/info/:mint', async (req, res, next) => {
  try {
    ...
    const pumpRes = await axios.get(
      'https://frontend-api.pump.fun/coins/' + req.params.mint,
      { timeout: 5000 }
    );
